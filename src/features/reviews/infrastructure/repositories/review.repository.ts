...
export class ReviewRepository extends BaseRepository<Review> implements IReviewRepository {
  ...
  async findWithFilters(
    ...
    try {
      ...
      // Query with joins using new foreign keys
      // Note: reviewer_profiles is joined via profiles (profiles.id = reviewer_profiles.user_id)
      let dataQuery = client
        ...
        .select(`
          *,
          ...
          photos:review_photos(id, url, caption, order),
          replies:review_replies(id, author_id, author_name, author_role, content, created_at),
          helpfulVotes:review_helpful(user_id, is_helpful)
        `);
      ...
      // Transform results with proper joins
      const reviews: ReviewWithRelations[] = (data || []).map((row: Record<string, unknown>) => {
        ...
        const reviewerProfile = author?.reviewer_profile as Record<string, unknown> | undefined;
        ...
        return {
          ...review,
          author: {
            id: (author?.id as string) || (row.author_id as string),
            name: (author?.display_name as string) || (row.author_name as string) || '',
            avatar: (author?.avatar_url as string) || (row.author_avatar as string) || null,
            level: reviewerProfile?.level as ReviewerLevel | undefined,
            badges: reviewerProfile?.badges as string[] | undefined,
          },
          photos: photos.map(p => ({
            id: p.id as string,
            reviewId: review.id,
  ...
  async findWithRelations(reviewId: string, currentUserId?: string): Promise<ReviewWithRelations | null> {
    try {
      ...
      // Query with joins using new foreign keys
      // Note: reviewer_profiles is joined via profiles
      const { data, error } = await client
        .from(this.tableName)
        .select(`
          *,
      ...
      return {
        ...review,
        author: {
          id: (author?.id as string) || (data.author_id as string),
          name: (author?.display_name as string) || (data.author_name as string) || '',
          avatar: (author?.avatar_url as string) || (data.author_avatar as string) || null,
          level: reviewerProfile?.level as ReviewerLevel | undefined,
          badges: reviewerProfile?.badges as string[] | undefined,
        },
        photos: photos.map(p => ({
