...
export class TopicRepository extends BaseRepository<Topic> implements ITopicRepository {
  ...
  async findWithAuthor(options?: {
    ...
    try {
      ...
      // Query with join using new foreign key
      let query = client
        .from(this.tableName)
        .select(`
          *,
  ...
  private toTopicWithAuthor(row: Record<string, unknown>): TopicWithAuthor {
    const dbRow = row as SupabaseTopic & { author?: { id: string; display_name: string | null; avatar_url: string | null } };
    ...
    return {
      ...
      createdAt: dbRow.created_at,
      updatedAt: dbRow.updated_at,
      authorName: dbRow.author?.display_name || dbRow.author_name || 'مستخدم',
      authorAvatar: dbRow.author?.avatar_url || dbRow.author_avatar || null,
    };
  }
}
...
export class ReplyRepository extends BaseRepository<ReplyEntity> implements IReplyRepository {
  ...
  private toReplyWithAuthor(row: Record<string, unknown>): ReplyWithAuthor {
    const dbRow = row as SupabaseReply & { author?: { id: string; display_name: string | null; avatar_url: string | null } };
